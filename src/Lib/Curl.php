...
class Curl
{
    ...
    public function httpGet($url, array $data = [])
    {
        ...
        return $curl->setGetParams($data)
            ->get($url);
    }

    public function httpPost($url, array $data = [], $options = [])
    {
        $curl = new Curl();

        return $curl->setPostParams($data)
            ->get($url);
    }
    ...
    public function getUrl()
    {
        if ( Count($this->_getParams) > 0 ) {
            ...
            if ( isset($parseUrl['query']) ) {
                ...
                $getParams = array_merge($output, $this->_getParams);

                return $parseUrl['scheme'] . '://' . $parseUrl['host'] . ( isset($parseUrl['path']) ? $parseUrl['path'] : '' ) . '?' . http_build_query($getParams);
            } else {
    ...
    public function setHeaders($headers)
    {
        if ( is_array($headers) ) {
            ...
            //set headers
            $this->setOption(CURLOPT_HTTPHEADER, $parsedHeader);
        }
